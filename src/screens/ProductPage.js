...
import { colors, navbtn, navbtnin, navbtnout,btn2 ,hr80,incdecbtn, incdecinput, incdecout } from '../globals/style';
import { necklace, earring ,watches ,Bracelet,ring,chain/*,goldColor ,silverColor*/ } from '../globals/style'
...
const data = route.params;
 ...
 const addtocard = () =>{
    ...
    const docRef = firebase.firestore().collection('UserCard').doc(firebase.auth().currentUser.uid);

    const data1 ={data, Addonquantity: addonquantity, Jewelryquantity:quantity}
...
docRef.get().then((doc) => {
  ...
  const carddata =JSON.stringify({card : [{Addonquantity:addonquantity,Jewelryquantity:quantity, data}]})
...
<View style={styles.s1}>
          <Image source={{
            ...
            <View style={styles.s2in}>
             <Text style={styles.text1}>{data.jewelryName}
             </Text>
...
<View style ={styles.container4}>
  <View style={styles.c4in}>
    ...
    {data.jewelryPrice != "" ? <Text style={styles.txt5}>
      Rs{((
        parseInt(data.jewelryPrice)*parseInt(quantity))+parseInt(addonquantity)*parseInt(data.jewelryAddonPrice)
      ).toString()}
      </Text> :
  ...
  <View style={hr80}></View>
   ...
   <View style={styles.btncont}>
    ...
    <TouchableOpacity style={btn2}>
      <Text style={styles.btntext} onPress={() => navigation.navigate('PlaceOdar', { carddata })}
      >Buy Now</Text>
    </TouchableOpacity>
   </View>
