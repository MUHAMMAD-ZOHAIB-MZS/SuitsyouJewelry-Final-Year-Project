...
import { PlaceOrder } from '../components/PlaceOdar';
...
const UserCard = ({ navigation }) => {
  ...
  const deleteItem = (item) => {
    const docRef = firebase.firestore().collection('UserCard').doc
            ...
            renderItem={({ item }) => {
              return (
                <View style={styles.cardcart}>
                  <Image source={{ uri: item.data.jewelryImageUrl }} style={styles.cardimg} />
                  <View style={styles.cardcartin}>
                    <View style={styles.c1}>
                      <Text style={styles.text1}>
                        {item.Jewelryquantity}&nbsp;{item.data.jewelryName}
                      </Text>
                      <Text style={styles.text2}>Rs{item.data.jewelryPrice}/each</Text>
                    </View>
                    {item.Addonquantity > 0 && (
                      <View style={styles.c2}>
                        <Text                         style={styles.text3}
                      >
                        {item.Addonquantity}&nbsp;&nbsp;{item.data.jewelryAddon}
                      </Text>
                      <Text style={styles.text3}>Rs{item.data.jewelryAddonPrice}/each</Text>
                    </View>
                  )}
                  <TouchableOpacity style={styles.c4} onPress={() =>  editItem(item)}>
                    <Text style={styles.text1}>Edit</Text>
                    <AntDesign name="edit" size={40} color="black" style={styles.del} />
                  </TouchableOpacity>
