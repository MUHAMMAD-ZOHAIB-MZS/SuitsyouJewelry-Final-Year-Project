...
import { colors, navbtn, navbtnin, navbtnout,btn1 ,hr80,incdecbtn, incdecinput, incdecout } from '../globals/style';
...
const placehow =() =>{
  const docRef = firebase.firestore().collection('UserOrders').
  ...
  return (
    ...
    <View style={styles.header}>
      ...
      <FlatList style={styles.c1} data={ordardata.card} 
      renderItem={
      ({item}) => {
        return(
        <View style={styles.rowut}>
          ...
          <View style={styles.left}>
               ...
               <Text style={styles.price1}>Rs{item.data.jewelryPrice}</Text>
          </View>
          <View style={styles.right}>
          <Text style={styles.totalprice}>Rs
          {parseInt(item.Jewelryquantity)* parseInt(item.data.jewelryPrice)}</Text>
          </View>
          </View>
        { item.Addonquantity > 0 && <View style={styles.row}>
          ...
          <Text style={styles.price1}>Rs{item.data.jewelryAddonPrice}</Text>
          </View>
          <View style={styles.right}>
           <Text style={styles.totalprice}>Rs
           {parseInt(item.Addonquantity) * parseInt(item.data.jewelryAddonPrice)}</Text>
          </View>
          </View>
          }
...
<View style={hr80}></View>
   ...
   <View>
    ...
    <Text style={styles.btntext} onPress ={() => placehow()}>Proceed to Payment</Text>
    </TouchableOpacity>
   </View>
